  ...
  return (
    <main className="app">
      <header className="app-header">
        ...
        <div className="header-actions"><button className="theme-toggle" aria-label={darkMode ? 'Use light mode' : 'Use dark mode'} title={darkMode ? 'Light mode' : 'Dark mode'} onClick={toggleTheme}><ThemeIcon dark={darkMode} /></button><button className={`edit-structure ${editingStructure ? 'active' : ''}`} onClick={() => { setEditingStructure((current) => !current); setEditingTabId(null); setEditingGroupId(null); setColorGroupId(null); setEditingId(null); setAdding(false); setAddingBulk(false) }}>{editingStructure ? 'Done' : 'Edit'}</button></div>
      </header>
          ...
          {tabGroups.length === 0 && !editingStructure ? <section className="empty"><h3>This workspace is empty</h3><p>Enter edit mode to create a deck.</p></section> : <>
            ...
            {tabGroups.length > 0 && (selectedGroup || allDecksSelected) && <section className="active-deck">
              <div className="study-launcher">
                {studyModeContext === currentStudyContext ? <div className="mode-picker"><div className="mode-chips"><button type="button" onClick={() => startStudying('front')}>Front → Back</button><button type="button" onClick={() => startStudying('alternating')}>Alternate ↔</button><button type="button" onClick={() => startStudying('back')}>Back → Front</button></div></div> : <button disabled={!studyCards.length} onClick={() => setStudyModeContext(currentStudyContext)}>{selectedGroup ? 'Start study' : 'Study all decks'} <ArrowIcon /></button>}
              </div>
              ...
              {selectedGroup && <><CardFilters value={cardFilter} onChange={chooseCardFilter} counts={filterCounts} actions={editingStructure ? <div className="add-card-actions"><button className="add-card" disabled={cardFilter !== 'all'} aria-label="Add card" title="Add card" onClick={() => { setAdding(true); setAddingBulk(false) }}><PlusIcon /></button><button className="add-card" disabled={cardFilter !== 'all'} aria-label="Import cards from JSON" title="Import cards from JSON" onClick={() => { setAdding(false); setAddingBulk(true) }}><ImportIcon /></button></div> : null} />
